...
L.Control.Search = L.Control.extend({
  ...
  options: {
    ...
    targetProperty: 'title',      //property in marker.options(or feature.properties for vector layer) trough filter elements in layer,
    displayProperty: 'title',      //property in marker.options(or feature.properties for vector layer)
  ...
  _defaultFilterData: function(text, records) {
    ...
    for(var key in records) {
      if( regSearch.test(records[key].target) )
  ...
  _defaultFormatData: function(json) {  //default callback for format data to indexed data
    var propName = this.options.targetProperty,
  ...
  _recordsFromLayer: function() {  //return table: key,value from layer
    var that = this,
      retRecords = {},
      propTarget = this.options.targetProperty,
      propResult = this.options.displayProperty,
      loc;
    ...
    this._layer.eachLayer(function(layer) {
      ...
      {
        try {
          if(_getPath(layer.options,propResult))
          {
            loc = layer.getLatLng();
            loc.layer = layer;
            retRecords[ _getPath(layer.options,propResult) ] = {
              target: _getPath(layer.options,propTarget),
              loc: loc
            };
            
          }
          else if(_getPath(layer.feature.properties,propResult)){
  
            loc = layer.getLatLng();
            loc.layer = layer;
            retRecords[ _getPath(layer.feature.properties,propResult) ] = {
              target: _getPath(layer.feature.properties,propTarget),
              loc: loc
            };
            
          }
          else
            throw new Error("propertyName '"+propResult+"' not found in marker");
      ...
      {
        try {
          if(layer.feature.properties.hasOwnProperty(propResult))
          {
            loc = layer.getBounds().getCenter();
            loc.layer = layer;      
            retRecords[ layer.feature.properties[propResult] ] = loc;
          }
          else
            throw new Error("propertyName '"+propResult+"' not found in feature");
        }
        catch(err){
          if (console) { console.warn(err); }
        }
